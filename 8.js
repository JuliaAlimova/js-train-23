...
class User {
  ...
  sendMessage = (message) => this.messenger.sendMessage(this.name, message); // Метод receiveMessage приймає аргументи user,message та виводить в консоль ${this.name} отримав повідомлення від ${user.name}: ${message}
...
class SMSMessenger {
  ...
  static sendMessage = (sender, message) =>
    console.log(`Відправлено SMS: [${sender}]: ${message}`);
}
...
class EmailMessenger {
  ...
  static sendMessage = (sender, message) => {
    console.log(`Відправлено Email: [${sender}]: ${message}`);
  };
}
